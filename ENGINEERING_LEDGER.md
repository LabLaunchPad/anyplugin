...
- **F19 — a measurement's UNIT is not its RESOLUTION.** `telemetry.test.ts` asserted `cpuUserMicros + cpuSystemMicros > 0` after a 3M-iteration loop. It passed on Linux and **failed on Windows with 0**. Not a flake and not contention: Windows accounts process CPU via `GetProcessTimes` at roughly the 15.6ms timer tick, so the loop finished inside a single tick and the counter legitimately read zero for work that definitely happened. **This is the same class the telemetry inventory itself documented** for `fsRead`/`fsWrite` — *"a field that reports 0 for work that definitely happened is worse than an absent field: it looks like an answer"* — and the inventory then asserted `cpuUsage` was *"microseconds on every supported platform"*. That claim was true about the **unit** and silently wrong about the **resolution**; `MEASURE_BEFORE_DEFINING_A_TELEMETRY_FIELD` was applied to the field's definition but never to the test's own assumption about it. **Fix**: the burn loop now runs on measured wall clock until it exceeds the coarsest accounting tick we support, so it is independent of machine speed (F17's lesson applied to a measurement rather than a timeout), and the universally-true properties — non-negative, never running backwards — are asserted **separately**, so a future resolution change cannot mask a genuinely broken counter. The inventory notes for both CPU fields now record resolution distinctly from unit. **Generalized**: for any measurement, unit, resolution, accounting method, and platform availability are four separate properties, and evidence for one is not evidence for another.
